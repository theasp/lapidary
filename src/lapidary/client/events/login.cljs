...
(defn login [{:keys [db]} [_ username password]]
  ...
  (when-not (get-in db [:login :loading?])
    {:db         (-> db
                     (update-in [:connections :http :login] inc)
                     (update :login merge {:username nil
                                           :password nil
                                           :loading? true}))
...
(defn login-ok [{:keys [db]} [_ result]]
  (let [{:keys [username password jwt]} result]
    ...
    {:dispatch [:refresh]
     :db       (-> db
                   (update-in [:connections :http :login] dec)
                   (update :login merge {:username username
                                         :password password
                                         :jwt      jwt
                                         :loading? false
                                         :error    nil}))}))
